...
#elif defined(CIPHER_TWOFISH_STATIC)
...
#if defined(CIPHER_TWOFISH_USE_LOCAL_HEADER)
#	include "Twofish.h"
#else
...
CIPHER_TWOFISH_API
void twofish_set_key(Twofish *object, const zuint8 *key, zusize key_size)
	{
	zsint i, j, k;
...
CIPHER_TWOFISH_API
void twofish_encipher(Twofish *object, const zuint32 *block, zusize block_size, zuint32 *output)
	{
	zuint32 a, b, c, d, x, y;
...
CIPHER_TWOFISH_API
void twofish_decipher(Twofish *object, const zuint32 *block, zusize block_size, zuint32 *output)
	{
	zuint32 a, b, c, d, x, y;
...
#if defined(CIPHER_TWOFISH_BUILD_MODULE_ABI)

#	include <Z/ABIs/generic/module.h>

	static ZModuleUnit const unit = {
		"Twofish", "Twofish", Z_VERSION(1, 0, 0), &abi_cipher_twofish
	};
